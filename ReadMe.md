# 🧬 LuckyGenesMDx - macOS Setup Guide
...
# Install MAMP
# Place project in: /Applications/MAMP/htdocs/LuckyGenesMDx
# Start MAMP servers
# Access at: http://localhost:8888/LuckyGenesMDx
```
...
sudo cp -r LuckyGenesMDx /Library/WebServer/Documents/

# Access at: http://localhost/LuckyGenesMDx
```
...
```sql
-- Create database
CREATE DATABASE LuckyGenesMDx_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Create dedicated user (recommended)
CREATE USER 'LuckyGenesMDx'@'localhost' IDENTIFIED BY 'your_secure_password';

-- Grant privileges
GRANT ALL PRIVILEGES ON LuckyGenesMDx_db.* TO 'LuckyGenesMDx'@'localhost';
...
mysql -u LuckyGenesMDx -p LuckyGenesMDx_db < database_schema.sql
...
```bash
# Login to MySQL
mysql -u LuckyGenesMDx -p LuckyGenesMDx_db
...
define('DB_NAME', 'LuckyGenesMDx_db');
define('DB_USER', 'LuckyGenesMDx');  // Or your MySQL username
...
// If using MAMP:
define('SITE_URL', 'http://localhost:8888/LuckyGenesMDx');

// If using Apache:
define('SITE_URL', 'http://localhost/LuckyGenesMDx');
```
...
http://localhost:8888/LuckyGenesMDx # MAMP
http://localhost/LuckyGenesMDx      # Apache
```
...
# Navigate to project
cd ~/Desktop/LuckyGenesMDx  # Adjust path as needed

# Create database and user
mysql -u root -p << EOF
CREATE DATABASE LuckyGenesMDx_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'LuckyGenesMDx'@'localhost' IDENTIFIED BY 'secure_password_here';
GRANT ALL PRIVILEGES ON LuckyGenesMDx_db.* TO 'LuckyGenesMDx'@'localhost';
FLUSH PRIVILEGES;
EOF
...
```bash
alias luckygene='cd ~/Desktop/LuckyGenesMDx && php -S localhost:8000'
...
- [ ] Database `LuckyGenesMDx_db` created
...
Your Mac is now set up for LuckyGenesMDx development. You can:
